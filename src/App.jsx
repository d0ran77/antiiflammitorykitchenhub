    ...
    <div className="min-h-screen relative font-body text-[#1a1a1a] overflow-x-hidden selection:bg-[#f28d35]/30">
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-4 pb-16">
        {activeTab === 'home' && (
          <div className="animate-in fade-in duration-1000">
            {/* Home Hero Section */}
            <section className="min-h-[40vh] md:min-h-[60vh] flex flex-col items-center justify-center text-center py-12 md:py-20 px-4">
              ...
              <p className="text-xl md:text-2xl text-[#1a1a1a] font-bold max-w-2xl mx-auto mb-8 font-body opacity-80">
                Harness the power of natural ingredients to boost immunity and reclaim your vitality.
              </p>
            </section>

            {/* Mobile Carousel - Now Slimmer and Longer */}
            <div className="flex overflow-x-auto snap-x snap-mandatory hide-scrollbar gap-6 md:grid md:grid-cols-4 pb-8 px-2 md:px-0">
              {categories.slice(0, 4).map(cat => (
                <button 
                  key={cat.id}
                  onClick={() => setActiveTab(cat.id)}
                  className="flex-shrink-0 w-[65vw] md:w-auto group relative bg-[#1a1a1a]/90 backdrop-blur-xl rounded-[3.5rem] shadow-2xl hover:shadow-[#f28d35]/20 snap-center transition-all border border-white/10 text-left overflow-hidden h-[35rem] md:h-[26rem] hover:-translate-y-2"
                >
        ...
        {['smoothies', 'breakfast', 'lunch', 'dinner', 'ingredients'].includes(activeTab) && (
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-600 pt-20">
            <header className="flex flex-col md:flex-row md:items-end justify-between mb-20 border-l-[10px] border-[#f28d35] pl-10">
              <div className="max-w-2xl">
                ...
                <p className="text-[#1a1a1a] font-bold text-2xl leading-relaxed font-body opacity-70">{activeCategoryData?.description}</p>
              </div>
            </header>

            {activeTab === 'ingredients' ? (
              <article className="bg-[#e8e7e7]/80 backdrop-blur-3xl rounded-[4rem] p-14 shadow-2xl border border-white">
                <IngredientSection 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {data[activeTab].map((item, idx) => (
                  <article key={idx} className="bg-[#1a1a1a]/85 backdrop-blur-xl rounded-[4rem] p-12 shadow-2xl border border-white/10 flex flex-col h-full hover:shadow-[#f28d35]/10 hover:-translate-y-3 transition-all duration-500">
                    <div className="flex justify-between items-start mb-10">
                      <h3 className="text-4xl font-black text-[#e8e7e7] leading-tight font-heading tracking-tighter">{item.title}</h3>
                      <div className="p-5 bg-[#f28d35] rounded-3xl text-[#1a1a1a] shadow-2xl">{item.icon}</div>
                    </div>
                    
                    <div className="flex items-center text-[10px] font-black uppercase tracking-widest text-[#1a1a1a] bg-[#f28d35] px-6 py-3 rounded-full w-fit mb-12 font-heading shadow-lg shadow-[#f28d35]/20">
                      <Info className="w-4 h-4 mr-2.5" />
                      {item.benefit}
                    </div>

                    <div className="mb-14 flex-grow">
                      <h4 className="text-[10px] font-black text-white/30 uppercase tracking-[0.4em] mb-8 font-heading">The Ingredients</h4>
                      <ul className="space-y-5">
                        {item.ingredients.map((ing, i) => (
                          <li key={i} className="text-[#e8e7e7] text-lg font-bold flex items-center font-body opacity-80 hover:opacity-100 transition-opacity">
                            <div className="w-2.5 h-2.5 bg-[#f28d35] rounded-full mr-5 shadow-sm"></div>
                            {ing}
                          </li>
                        ))}
                      </ul>
                    ...
                    <button 
                      ...
                      className="w-full py-6 bg-white/5 text-[#f28d35] border-2 border-[#f28d35] text-lg font-black rounded-3xl hover:bg-[#f28d35] hover:text-[#1a1a1a] shadow-2xl transition-all flex items-center justify-center active:scale-95 font-heading tracking-tighter"
                    >
                      START THE METHOD
                    </button>
                  </article>
      ...
      {selectedRecipe && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-8">
          ...
          <div className="relative bg-[#e8e7e7]/90 backdrop-blur-3xl w-full max-w-3xl rounded-[5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-400 border-4 border-[#1a1a1a]">
            <div className="p-12 sm:p-16 border-b-2 border-[#1a1a1a]/10 flex justify-between items-center bg-white/50 backdrop-blur-md">
              <h3 className="text-4xl sm:text-5xl font-black text-[#1a1a1a] tracking-tighter font-heading leading-none">{selectedRecipe.title}</h3>
              <button 
                onClick={() => setSelectedRecipe(null)}
                className="p-5 bg-[#1a1a1a] text-[#f28d35] rounded-3xl transition-all shadow-2xl active:scale-90 hover:rotate-90 duration-300"
              >
                <X className="w-8 h-8" />
              </button>
            </div>
            <div className="p-12 sm:p-16 max-h-[70vh] overflow-y-auto custom-scrollbar">
              <div className="mb-16">
                <h4 className="text-[#1a1a1a] font-black uppercase text-xs tracking-[0.5em] mb-12 flex items-center font-heading opacity-40">
                  <Utensils className="w-5 h-5 mr-4" /> THE COOKING METHOD
                </h4>
                <div className="space-y-16">
                  {selectedRecipe.instructions.map((step, idx) => (
                    <div key={idx} className="flex space-x-10">
                      <div className="flex-shrink-0 w-16 h-16 rounded-[2rem] bg-[#1a1a1a] text-[#f28d35] flex items-center justify-center font-black text-3xl shadow-2xl font-heading">
                        {idx + 1}
                      </div>
              ...
              <div className="bg-[#1a1a1a] text-[#f28d35] rounded-[3.5rem] p-12 shadow-2xl border border-white/5">
                <h4 className="text-white/20 font-black uppercase text-[10px] tracking-[0.4em] mb-5 font-heading">Digital Health Benefit</h4>
                <p className="text-2xl font-black flex items-start leading-snug font-heading tracking-tight">
                  <Info className="w-8 h-8 mr-5 text-white mt-0.5 flex-shrink-0" />
